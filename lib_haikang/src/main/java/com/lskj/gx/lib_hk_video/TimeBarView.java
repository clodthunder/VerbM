...
public class TimeBarView extends View {
    ...
    private void initView() {
        //下面的值是初始化后就能确定的
        mMiddleLineDrawable = ResourcesCompat.getDrawable(getContext().getResources(), R.drawable.hk_time_bar_center_line, getContext().getTheme());
        //小刻度大小，大刻度高度 = 小刻度高度*2
    ...
    private void drawVideoArea(Canvas canvas) {
        ...
        for (RecordSegment recordSegment : mFileInfoList) {
            //给画笔设置颜色
            mVideoPaint.setColor(ContextCompat.getColor(getContext(), R.color.hk_playback_timebar_color));
