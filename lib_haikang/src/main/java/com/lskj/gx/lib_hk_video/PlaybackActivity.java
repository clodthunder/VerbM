...
import com.lskj.gx.lib_hk_video.widget.AutoHideView;
import com.lskj.gx.lib_hk_video.widget.PlayWindowContainer;
 ...
 */
@Route(path = "/hk_video/play_back_activity") public class PlaybackActivity extends AppCompatActivity
    implements View.OnClickListener, HikVideoPlayerCallback, TextureView.SurfaceTextureListener {
  private static final String TAG = "PlaybackActivity";
  //    private static final String playbackUri = "rtsp://120.236.240.174:554/openUrl/IkuEDCg";
  private static final String playbackUri = "rtsp://39.96.32.210:554/openUrl/ByBVy5W";
  ...
  private void initPlayWindowContainer() {
    ...
    frameLayout.setOnDigitalListener(new PlayWindowContainer.OnDigitalZoomListener() {
      @Override public void onDigitalZoomOpen() {
        executeDigitalZoom();
      }
    });
  }
  ...
  private void initTimeBarView() {
    ...
    timeBar.setTimeBarCallback(new TimeBarView.TimePickedCallBack() {
      @Override public void onMoveTimeCallback(long currentTime) {

      }

      @Override public void onBarMoving(long currentTime) {

      }

      @Override public void onTimePickedCallback(long currentTime) {
        //TODO 注意: 定位操作的时间要在录像片段开始时间和结束时间之内，不再范围内不要执行以下操作
        mSeekCalendar.setTimeInMillis(currentTime);
        Log.e(TAG,
            "onTimePickedCallback: currentTime = " + CalendarUtil.calendarToyyyy_MM_dd_T_HH_mm_SSSZ(mSeekCalendar));
      ...
      @Override public void onMaxScale() {

      }

      @Override public void onMinScale() {
  ...
  private void executeRecordEvent() {
    ...
    if (!mRecording) {
      ...
      if (mPlayer.startRecord(path)) {
        ...
        mRecording = true;
        recordButton.setText(R.string.hk_close_record);
      ...
      mRecording = false;
      recordButton.setText(R.string.hk_start_record);
    }
  }
  ...
  private void executeSoundEvent() {
    ...
    if (!mSoundOpen) {
      ...
      if (mPlayer.enableSound(true)) {
        ...
        mSoundOpen = true;
        soundButton.setText(R.string.hk_sound_close);
      }
    } else {
      //关闭声音
      if (mPlayer.enableSound(false)) {
        ...
        mSoundOpen = false;
        soundButton.setText(R.string.hk_sound_open);
      }
    }
  }
  ...
  private void executePauseEvent() {
    ...
    if (!mPausing) {
      ...
      if (mPlayer.pause()) {
        ...
        mPausing = true;
        pauseButton.setText(R.string.hk_resume);
      }
    } else {
      //恢复播放
      if (mPlayer.resume()) {
        ...
        mPausing = false;
        pauseButton.setText(R.string.hk_pause);
      }
    }
  }
  ...
  private void executeDigitalZoom() {
    ...
    if (!mDigitalZooming) {
      frameLayout.setOnScaleChangeListener(new PlayWindowContainer.OnDigitalScaleChangeListener() {
        @Override public void onDigitalScaleChange(float scale) {
        ...
        @Override public void onDigitalRectChange(CustomRect oRect, CustomRect curRect) {
  ...
  private void hideSystemUI() {
    ...
    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
      decorView.setSystemUiVisibility(
          View.SYSTEM_UI_FLAG_LOW_PROFILE | View.SYSTEM_UI_FLAG_HIDE_NAVIGATION // hide nav bar
   ...
   */
  @Override @WorkerThread public void onPlayerStatus(@NonNull HikVideoPlayerCallback.Status status, int errorCode) {
